...
Thank you for your interest in contributing to Qubee! This guide will help you get started and ensure a smooth collaboration.
...
| **New feature or architecture change** | Start a [GitHub Discussion](https://github.com/mikiasendale/Qubbe/discussions) **before** opening a PR |
...
| **Question** | Open a [GitHub Discussion](https://github.com/mikiasendale/Qubbe/discussions) |
...
Four packages under `packages/@qubee/` are published to npm: `dsl`, `storage`, `renderer`, and `importer`. Anything that ships inside one of those tarballs is under version control in the literal sense — the version number on npm has to keep meaning "this exact source".
...
Be deliberate with `@qubee/dsl`. It is the contract the other packages and downstream deployments validate against, so a change that narrows what an existing document may contain is a breaking change even when the diff looks small.

You never publish anything yourself. Once your PR is merged, a version that is not yet on the registry is released automatically, and a `@qubee/<name>@<version>` tag is written afterwards to record it. That tag is a marker, not a trigger: pushing one does not release anything.
...
Use the [Bug Report](https://github.com/mikiasendale/Qubbe/issues/new?template=bug_report.yml) issue template. Include:
...
Use the [Feature Request](https://github.com/mikiasendale/Qubbe/issues/new?template=feature_request.yml) issue template. For larger features, please open a [Discussion](https://github.com/mikiasendale/Qubbe/discussions) first.

## Security Vulnerabilities

Please report security vulnerabilities through [GitHub Security Advisories](https://github.com/mikiasendale/Qubbe/security/advisories/new). **Do not** open a public issue for security vulnerabilities.

## License

By contributing to Qubee, you agree that your contributions will be licensed under the [MIT License](LICENSE).
