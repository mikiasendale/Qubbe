...
import { DocumentNotFoundError, DocumentVersionError, type QubeeDocument } from '@qubee/storage';
...
function isStoreValidationError(error: unknown): error is Error {
  return (
    ...
    error.message.startsWith('@qubee/storage:') &&
...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  return withRequestOwnerId(req, async (ownerId, responseHeaders) => {
    ...
    try {
      ...
      await store.saveDocument({
        ...(body as QubeeDocument),
        stage: { ...(body as QubeeDocument).stage, updatedAt: Date.now() },
      });
    } catch (error) {
      return mapSaveError(error, responseHeaders);
    }
