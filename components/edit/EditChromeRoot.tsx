...
import { isQubeeEditorEnabled } from '@/lib/config/feature-flags';
...
export function EditChromeRoot({ scene, isEditable, onToggleEditMode }: EditChromeRootProps) {
  ...
  useEffect(() => {
    document.body.dataset.qubeeEditor = 'true';
    return () => {
      delete document.body.dataset.qubeeEditor;
    };
  }, []);
  ...
  const headerControls = inWorkbenchPanel ? undefined : (
    <HeaderControls
      mode="edit"
      canEdit={isEditable}
      onToggleEditMode={isQubeeEditorEnabled() && !inWorkbenchPanel ? onToggleEditMode : undefined}
    />
  );
