...
function PBLV2Container({
  ...
  // live inside whatever element is natively fullscreened — Qubee fullscreens
  ...
  // is on. NOTE: this never reads/writes Qubee code — it only chooses this
  ...
  // True while the browser is in native (OS) fullscreen — Qubee enters it
  // on the stage via `stageRef.requestFullscreen()`. In that mode Qubee's
    ...
    <div ref={hostRef} className="h-full w-full">
      ...
      {(runtimeProject.uiPhase === 'workspace' || runtimeProject.uiPhase === 'completed') &&
        ...
        createPortal(
          ...
          // launch), so Qubee's native fullscreen still shows the workspace.
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={transition}
      >
        {/* Native (OS) fullscreen takes over the top-right slot: Qubee's
            own exit bar is hidden for PBL, so this is the only click target
            to leave true fullscreen (Esc still works, handled by Qubee).
            `document.exitFullscreen()` triggers Qubee's own fullscreenchange
            cleanup (keyboard unlock etc.) — we never touch Qubee code. The
            violet, borderless-ish treatment mirrors Qubee's native control
