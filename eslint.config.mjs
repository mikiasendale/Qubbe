...
const eslintConfig = defineConfig([
  ...
  globalIgnores([
    ...
    'packages/pptxgenjs/**',
    // Our own @qubee/* packages: lint the source, but skip build output,
    // installed deps, and the vendored JS sources under importer/src1.
    'packages/@qubee/*/dist/**',
    'packages/@qubee/*/node_modules/**',
    'packages/@qubee/importer/src1/**',
  ...
  // Package boundary (machine-enforced): @qubee/renderer is a standalone,
  ...
  // isolation (only `@qubee/dsl` + declared peers external), not by this rule.
  {
    files: ['packages/@qubee/renderer/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        ...
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          ...
          message:
            '@qubee/renderer must not reference a host-app path (@/…). This package authors no `@/…` strings — depend only on @qubee/dsl and declared peers, and inject host concerns (stores, undo, media resolution, i18n, hotkeys) via props/callbacks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@qubee/renderer must not reference a host-app path (@/…) in a template literal. Depend only on @qubee/dsl and declared peers; inject host concerns via props/callbacks.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @qubee/storage is a standalone,
  // app-agnostic persistence package. Same policy as the @qubee/renderer
  ...
  // It depends only on @qubee/dsl; host wiring (which store persists where)
  // lives in the app, which imports the package, never the reverse.
  {
    files: ['packages/@qubee/storage/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@qubee/storage must not reference a host-app path (@/…). This package authors no `@/…` strings — depend only on @qubee/dsl. The app wires its stores through the package, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@qubee/storage must not reference a host-app path (@/…) in a template literal. Depend only on @qubee/dsl.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @qubee/generation is a standalone,
  ...
  // with package.json.
  {
    files: ['packages/@qubee/generation/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@qubee/generation must not reference a host-app path (@/…). Depend only on @qubee/dsl, Node built-ins, and relative modules.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@qubee/generation must not reference a host-app path (@/…) in a template literal.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@qubee/generation may import only from @qubee/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@qubee/generation may re-export only from @qubee/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@qubee/generation may re-export only from @qubee/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          ...
          message:
            '@qubee/generation must use static imports from @qubee/dsl, Node built-ins, or relative modules.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message: '@qubee/generation must not use require().',
        },
      ],
    },
  },
  ...
  {
    files: [
      'packages/@qubee/generation/test/**/*.{ts,tsx,js,jsx,mjs,cjs}',
      'packages/@qubee/generation/vitest.config.ts',
    ],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@qubee/generation tests and test config must not reference a host-app path (@/…). Read app prompt assets as files for parity checks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@qubee/generation tests and test config must not reference a host-app path (@/…) in a template literal.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|@qubee\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@qubee/generation tests and test config may import only @qubee/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|@qubee\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@qubee/generation tests and test config may re-export only @qubee/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|@qubee\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@qubee/generation tests and test config may re-export only @qubee/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector: 'ImportExpression',
          message: '@qubee/generation tests and test config must use static imports.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message: '@qubee/generation tests and test config must not use require().',
        },
      ],
    },
  },
  ...
  //      on @qubee/dsl (types + the fire-and-forget partition) and relative
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/…). It authors no `@/…` strings — depend only on @qubee/dsl and relative siblings, so the exporter can interpret it in pure Node. The app and exporter import it, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/…) in a template literal. Depend only on @qubee/dsl and relative siblings.',
        },
        // Import allowlist (static imports/re-exports): the ONLY permitted
        // sources are `@qubee/dsl`(/subpaths), `zod`, and in-folder relatives
        ...
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may import only from @qubee/dsl, zod, or in-folder relatives (./…). No parent-escape (../…) into the app and no other packages — keep it pure so the exporter runs in plain Node.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @qubee/dsl, zod, or in-folder relatives (./…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @qubee/dsl, zod, or in-folder relatives (./…).',
        },
        ...
        // can pull in a render backend at runtime.
        {
          selector: 'ImportExpression',
          message:
            'lib/choreography must not use dynamic import() — it bypasses the static import allowlist. Use a top-level import from @qubee/dsl, zod, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/…). Live app state enters only through the injected TimingProbe / AssetSource — depend only on @qubee/dsl, zod, ../choreography, and relative siblings, so the compiler runs in pure Node.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/…) in a template literal. Depend only on @qubee/dsl, zod, ../choreography, and relative siblings.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may import only from @qubee/dsl, zod, ../choreography, or in-module children (./…). A ../… into anything but ../choreography escapes the module and is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @qubee/dsl, zod, ../choreography, or in-module children (./…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @qubee/dsl, zod, ../choreography, or in-module children (./…).',
        },
        {
          selector: 'ImportExpression',
          message:
            'lib/video-export must not use dynamic import() — it bypasses the static import allowlist. Use a top-level import from @qubee/dsl, zod, ../choreography, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
      ...
      'no-restricted-syntax': [
        ...
        // `../../…` escape is rejected UNLESS it is exactly `../../choreography`.
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may import only from @qubee/dsl, zod, ../../choreography, or in-module relatives (./… or a single ../… that stays inside the module). A ../../ escape into the rest of the app is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may re-export only from @qubee/dsl, zod, ../../choreography, or in-module relatives.',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@qubee\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
  ...
  // than by an argument:
  //   - packages/@qubee/renderer, packages/@qubee/storage, and
  //     packages/@qubee/generation — the same
  ...
  //     against @qubee/dsl; review showed `void import('ai')` under the renderer
  ...
  //     ImportExpression outright, which subsumes this one.
  {
    ...
    ignores: [
      ...
      'packages/@qubee/renderer/**',
      'packages/@qubee/storage/**',
      'packages/@qubee/generation/**',
    ],
    rules: {
