/**
 * QUBEE Agent — pi StreamFn adapter (promoted from PoC).
 *
 * Bridges the pi agent loop's LLM call to Qubee's existing AI-SDK-based
 ...
 * call through Qubee's resolved Vercel `LanguageModel`, then map the AI SDK
...
/** Build a pi `StreamFn` that calls Qubee's connector instead of pi-ai providers. */
