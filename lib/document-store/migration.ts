import { DSL_VERSION, migrate } from '@qubee/dsl';
import { BrowserKVStore, type DocumentStore, type KVStore } from '@qubee/storage';
...
export function documentLockName(stageId: string): string {
  return `qubee:document:${encodeURIComponent(stageId)}`;
}
 ...
 * This mirrors `@qubee/storage`'s private `migrateDocument` in
