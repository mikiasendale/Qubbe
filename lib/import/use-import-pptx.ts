...
// pdfjs-dist (transitively pulled by `qubee-importer/src`) uses dynamic
...
import type * as QubeeImport from '@qubee/importer';

const log = createLogger('ImportPptx');

export type PptxUpload = NonNullable<QubeeImport.ImportPptxOptions['upload']>;
 ...
 * the bundled `qubee-importer` dist that we load by URL to bypass
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // `scripts/sync-qubee-importer.mjs` copies the prebuilt dist into
        // `public/vendor/` after every `pnpm install`.
        const url = '/vendor/qubee-importer/index.js';
