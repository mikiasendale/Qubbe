...
app-only outline modes. Generation templates live in `@qubee/generation`.
...
- Package-owned generation templates follow the same placeholder conventions;
  edit them under `packages/@qubee/generation/templates/`.
...
- Tests in `tests/prompts/templates.test.ts` and
  `packages/@qubee/generation/test/` assert that fully rendered app and
