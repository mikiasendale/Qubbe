...
// VideoManifest / SlideContent / QuizContent / …) now lives in `@qubee/dsl` and
...
export { isSlideContent, isQuizContent } from '@qubee/dsl';

// `@qubee/dsl` inlines the question-type union on `QuizQuestion.type` rather than
// exporting a named alias; derive it here so editor quiz code can keep importing
// `QuizQuestionType` from `@/lib/types/stage`.
export type QuizQuestionType = import('@qubee/dsl').QuizQuestion['type'];
...
export type { SceneContent as SceneContentBase } from '@qubee/dsl';
...
export type { Scene as SceneShape } from '@qubee/dsl';
...
export type AppScene = DslScene<Action, SceneContent> & {
   ...
   * annotation only — not part of the `@qubee/dsl` Scene contract. Absent on
