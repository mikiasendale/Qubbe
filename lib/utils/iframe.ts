 ...
 * `{ __qubeeErrorReplayRequest: true }` message once its listener is ready. The
...
(function () {
  ...
  function emit(errorKind, message) {
    try {
      window.parent.postMessage(
        { __qubeeInteractive: true, kind: 'runtime-error', errorKind: errorKind, message: message },
        '*'
      );
    } catch (e) {}
  }
  ...
  window.addEventListener('message', function (e) {
    var d = e && e.data;
    if (d && d.__qubeeErrorReplayRequest === true) {
...
(function () {
  if (window.__qubeeElementPickerInstalled) return;
  window.__qubeeElementPickerInstalled = true;
  var armed = false;
  var selectors = [];
  var root = null;
  var hoverBox = null;
  ...
  function ensureRoot() {
    ...
    root.setAttribute('data-qubee-element-picker-overlay', '');
  ...
  function isOverlay(element) {
    return !!(element && element.closest && element.closest('[data-qubee-element-picker-overlay]'));
  }
  ...
  function draw() {
    ...
    Array.prototype.slice.call(root.querySelectorAll('[data-qubee-picker-pin]')).forEach(function (node) { node.remove(); });
    selectors.forEach(function (selector, index) {
      var element = null;
      try { element = document.querySelector(selector); } catch (e) {}
      if (!selectable(element)) return;
      var badge = document.createElement('div');
      badge.setAttribute('data-qubee-picker-pin', '');
  ...
  function onKey(event) {
    ...
    event.stopImmediatePropagation();
    disarm();
    emit({ __qubeeInteractive: true, kind: 'element-picker-disarmed' });
  }
  function arm() {
