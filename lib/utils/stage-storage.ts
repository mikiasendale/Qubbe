...
import { DocumentVersionError, type DocumentSummary } from '@qubee/storage';
...
type ThumbnailSlide = import('@qubee/dsl').Slide;
            ...
            for (const el of mediaElements) {
              const videoBinding =
                el.type === 'video'
                  ? resolveVideoMediaForElement(
                      taskEntries,
                      el as import('@qubee/dsl').PPTVideoElement,
                      stageId,
                      documentElements,
                    )
                  : undefined;
              ...
              const selected =
                el.type === 'video'
                  ...
                  : resolveMediaTaskForElement(
                      taskEntries,
                      el as import('@qubee/dsl').PPTElement,
                      stageId,
                    );
...
// `@qubee/storage` DocumentStore. With server persistence on (`listStages`
