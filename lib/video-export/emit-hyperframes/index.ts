...
export interface EmitHyperframesOptions {
  ...
  height?: number;
  /** Composition id used for the root `data-composition-id` and the timeline key. Default `qubee`. */
  compositionId?: string;
  /** Relative path the emitted HTML loads GSAP from. Default `assets/vendor/gsap.min.js`. */
  gsapVendorPath?: string;
  /** Manifest filename. Default `qubee-video-manifest.json`. */
  manifestPath?: string;
...
const DEFAULT_MANIFEST = 'qubee-video-manifest.json';
...
function interactiveStaticBridgeScript(labels: InteractiveFallbackLabels): string {
  ...
  return `
function initializeQubeeInteractiveStaticFrames() {
  var hosts = Array.from(document.querySelectorAll('[data-interactive-static-host]'));
  window.__qubeeVideoDiagnostics = window.__qubeeVideoDiagnostics || [];
  window.__qubeeVideoManifest = window.__qubeeVideoManifest || { runtimeDiagnostics: [] };
  var labels = ${localized};
  var diagnosticCodes = new Set(${diagnosticCodes});
  var runtimeReport = document.querySelector('[data-qubee-runtime-diagnostics]');
  function record(sceneId, code, message) {
    ...
    window.__qubeeVideoDiagnostics.push(diagnostic);
    window.__qubeeVideoManifest.runtimeDiagnostics = window.__qubeeVideoDiagnostics.slice();
    if (runtimeReport) runtimeReport.textContent = JSON.stringify(window.__qubeeVideoDiagnostics);
  ...
  return Promise.all(hosts.map(function (host) {
    return new Promise(function (resolve) {
      ...
      function onMessage(event) {
        ...
        var data = event.data || {};
        if (data.__qubeeInteractive === true && data.kind === 'runtime-error') {
      ...
      iframe.addEventListener('load', function () {
        loaded = true;
        try { iframe.contentWindow.postMessage({ __qubeeErrorReplayRequest: true }, '*'); } catch (_) {}
      }, { once: true });
...
function renderReadme(project: {
  ...
  const scriptFontSummary = project.quizScriptFonts.length
    ...
    : '';
  return `# ${project.stageName} — Qubee video export
...
- Runtime interactive diagnostics are exposed live through \`window.__qubeeVideoManifest.runtimeDiagnostics\` and the machine-readable DOM report in \`index.html\`.
...
export function emitHyperframes(
  ...
  const compositionId = options.compositionId ?? 'qubee';
...
<title>${escapeHtml(ir.stage.name)} — Qubee video</title>
<style>
  ${INTER_FONT_FACE_CSS}${
    hasQuizQuestionList
...
${effectHtml.join('\n')}
${subtitles.html}
<script type="application/json" data-qubee-runtime-diagnostics>[]</script>
</div>
...
${statements.join('\n')}
window.__qubeeVideoManifest = { runtimeDiagnostics: [], manifestPath: ${JSON.stringify(manifestPath)} };
window.__timelines = window.__timelines || {};
${
  hasInteractiveHtml
    ? `${interactiveStaticBridgeScript(labels.interactive)}
window.__qubeeInteractiveReady = initializeQubeeInteractiveStaticFrames();
window.__qubeeInteractiveReady.then(function () {
