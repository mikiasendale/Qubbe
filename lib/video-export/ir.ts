 ...
 *
 * Pure: depends only on `@qubee/dsl` (the `SceneType` set) and `zod`.
 */
import { z } from 'zod';
import { SCENE_TYPES } from '@qubee/dsl';

/** Manifest `schema` tag — stable across versions; the shape is versioned by {@link VIDEO_TIMELINE_VERSION}. */
export const VIDEO_TIMELINE_SCHEMA = 'qubee.videoTimeline';
...
export const VIDEO_TIMELINE_COMPILER = 'qubee-video-timeline';
