...
The **contract keystone** of the QUBEE SDK family. `@qubee/dsl` is *pure spec* — the
...
package can depend on `@qubee/dsl` without pulling in junk.

## Dependency arrows (acyclic)

```
@qubee/dsl       ->  (nothing)
@qubee/renderer  ->  @qubee/dsl
@qubee/importer  ->  @qubee/dsl
@qubee/exporter  ->  @qubee/dsl     (reserved, future)
```

`@qubee/dsl` is the only package everything else depends on, and it depends on
nothing.
...
```ts
import type { Slide, PPTElement, Action } from '@qubee/dsl';
import { isTextElement, DSL_VERSION, SYNC_ACTIONS } from '@qubee/dsl';
```
...
1. **JSON Schema artifacts (cross-language mirror)** — `Stage`, the default
   ...
   ```ts
   import stageSchema from '@qubee/dsl/schema/stage.schema.json' with { type: 'json' };
   import sceneSchema from '@qubee/dsl/schema/scene.schema.json' with { type: 'json' };
   import actionSchema from '@qubee/dsl/schema/action.schema.json' with { type: 'json' };
...
2. **Pure validators (in-process boundary)** — `validate*` are hand-written,
   ...
   ```ts
   import { validateStage, validateScene, validateAction } from '@qubee/dsl';
...
3. **Pure normalizers (repair boundary)** — where `validate*` *reports* on a
   ...
   ```ts
   import { normalizeElement, normalizeScene, ELEMENT_DEFAULTS } from '@qubee/dsl';
...
```ts
import { DSL_VERSION, migrate, dslVersionOf, needsMigration } from '@qubee/dsl';
...
outside the document, per learner, through a `RuntimeStore` (`@qubee/storage`).
...
Both consumers are now wired to `@qubee/dsl` and no longer vendor their own copy
of the slide types:

- **`@qubee/importer`**: imports all slide types from `@qubee/dsl`; vendored
  `qubee/types/slides.ts` deleted. The importer emits complete DSL `Slide`
  objects directly (the old partial "draft slide" + post-fill step is gone).
- **`@qubee/renderer`**: imports all slide types from `@qubee/dsl`; vendored
  `types/slides.ts` deleted. `@qubee/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@qubee/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@qubee/importer` to import types from `@qubee/dsl` (vendored copy deleted).
- [x] Wire `@qubee/renderer` to import types from `@qubee/dsl` (vendored copy deleted).
...
- [ ] Reserve `@qubee/exporter` as the 4th family member.
...
```ts
import type { Action, InteractiveContent, PBLContent, Scene, SceneContent, WidgetConfigBase } from '@qubee/dsl';
...
```bash
pnpm --filter @qubee/dsl build         # -> dist/ (index.js, index.d.ts, …) + dist/schema/*.json
pnpm --filter @qubee/dsl build:schema  # regenerate only dist/schema/*.json
pnpm --filter @qubee/dsl typecheck
pnpm --filter @qubee/dsl test
```

## License

MIT, matching the rest of the family (`@qubee/dsl`, `@qubee/importer`,
`@qubee/renderer`) and the Qubee root, so the license policy is uniform
across the SDK.
