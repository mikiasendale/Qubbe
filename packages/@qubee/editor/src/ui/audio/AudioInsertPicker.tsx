...
export function AudioInsertPicker({ labels, onInsert }: AudioInsertPickerProps) {
  ...
  return (
    <div className="qubee-editing-ui-audio-insert-picker">
      <button
        type="button"
        className="qubee-editing-ui-audio-dropzone"
      ...
      <input
        ...
        type="file"
        accept="audio/*"
        className="qubee-editing-ui-visually-hidden"
        onChange={(event) => void insertFile(event.target.files?.[0])}
      />
      <div className="qubee-editing-ui-audio-or">{labels.audioOr}</div>
      <div className="qubee-editing-ui-audio-url-row">
        <input
          type="url"
          value={url}
