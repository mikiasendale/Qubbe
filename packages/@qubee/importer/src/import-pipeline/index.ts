/**
 * High-level entry: parsed pptxtojson(-pro) JSON → Qubee canvas `Slide[]`.
 *
 ...
 * whole import either.
 */
import { normalizeSlideWith, type Slide } from '@qubee/dsl';
...
/**
 * Convert a pre-parsed pptxtojson(-pro) `Output` JSON into Qubee slides.
 *
...
const normalizeImportedSlide = normalizeSlideWith({
  ...
  onDropped: (_element, error) => {
    console.warn(
      `[@qubee/importer] dropping element that fails DSL normalization: ${
...
/**
 * Parse a .pptx file and convert it into Qubee canvas slides.
 *
