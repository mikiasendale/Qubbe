...
React component for rendering PPTist-style `Slide` JSON. Extracted from [Qubee](https://github.com/mikiasendale/Qubbe).

> `@qubee/renderer` is the read-only canvas package. Editing lives in the
> separate `@qubee/editor` package, which depends on this renderer.

## Migrating from 0.0.x

Version `0.1.0` removes the experimental `@qubee/renderer/editing` subpath.
Install `@qubee/editor` and migrate editing imports to its explicit layers:

```ts
import { EditableSlideCanvas } from '@qubee/editor/react';
import { EditableSlideCanvasWithUI } from '@qubee/editor/ui';
```

Read-only rendering imports from `@qubee/renderer` are unchanged.

## Install

```bash
pnpm add @qubee/renderer
# or
npm install @qubee/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@qubee/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@qubee/renderer';
...
### Granular components — `@qubee/renderer/elements`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@qubee/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
[`@qubee/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @qubee/renderer` end-to-end.
