...
The QUBEE pluggable persistence layer: small, swappable-backend primitives for
persisting app state, depending only on [`@qubee/dsl`](../dsl).
...
| `StorageProvider` (from `@qubee/dsl`) | the asset seam: `put(blob) → ref`, `resolve(ref) → url`, `remove(ref)` | `BrowserAssetStore` over IndexedDB (`assets` registry + `blobs`) + object URLs |
...
data lives in the new `qubee-asset-pool` database.
...
- `BrowserAssetStore` deliberately does not read data written by the 0.1.x
  provider in `qubee-assets`. Its content-addressed `sha256-` refs are no longer
...
- [x] `StorageProvider` (in `@qubee/dsl`) + browser asset registry: allocated
