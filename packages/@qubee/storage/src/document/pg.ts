...
import type { Scene, Stage } from '@qubee/dsl';
...
-- current_setting('qubee.suppress_stage_notify', true) before pg_notify.
-- Batch/backfill writers MUST run SET LOCAL qubee.suppress_stage_notify =
...
CREATE OR REPLACE FUNCTION qubee_bump_scene_revision() RETURNS trigger AS $$
DECLARE
  v_stage_id text;
  v_scene_id text;
BEGIN
  ...
  IF coalesce(current_setting('qubee.suppress_stage_notify', true), '') <> 'on' THEN
    PERFORM pg_notify('qubee_agent_event_wakeup', json_build_object('kind', 'stage', 'stageId', v_stage_id)::text);
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION qubee_bump_stage_revision() RETURNS trigger AS $$
DECLARE
  v_stage_id text;
BEGIN
  IF TG_OP = 'DELETE' THEN
...
DROP TRIGGER IF EXISTS qubee_scene_revision_trigger ON document_scenes;

CREATE TRIGGER qubee_scene_revision_trigger
AFTER INSERT OR UPDATE OR DELETE ON document_scenes
FOR EACH ROW EXECUTE FUNCTION qubee_bump_scene_revision();

DROP TRIGGER IF EXISTS qubee_stage_revision_trigger ON document_stages;

CREATE TRIGGER qubee_stage_revision_trigger
AFTER INSERT OR UPDATE OR DELETE ON document_stages
FOR EACH ROW EXECUTE FUNCTION qubee_bump_stage_revision();
`;
...
function assertValid(
  ...
  throw new Error(`@qubee/storage: invalid ${label}: ${detail}`);
}
...
function encodeJson(value: unknown, label: string): string {
  try {
    ...
    throw new Error(`@qubee/storage: ${label} is not JSON-serializable`, { cause: error });
  }
}
...
function migrateDocument<TScene extends SceneLike, TStage extends Stage>(
  doc: QubeeDocument<TScene, TStage>,
): QubeeDocument<TScene, TStage> {
  const { outline, ...core } = doc;
  const migrated = migrate(core) as QubeeDocument<TScene, TStage>;
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof candidate.id !== 'string') {
    throw new Error(
      `@qubee/storage: scene id must be a string, got ${JSON.stringify(candidate.id)}`,
    );
  }
  if (candidate.stageId !== stageId) {
    throw new Error(
      `@qubee/storage: scene ${JSON.stringify(candidate.id)} has stageId ` +
  ...
  if (typeof candidate.order !== 'number' || !Number.isFinite(candidate.order)) {
    throw new Error(
      `@qubee/storage: scene ${JSON.stringify(candidate.id)} order must be a finite number, ` +
  ...
  implements DocumentStore<TScene, TStage>, DocumentFolderStore, StageFreshnessManifestStore
{
  ...
  constructor(queryable: Queryable, options: PgDocumentStoreOptions) {
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@qubee/storage: withTransaction is required and must pin a fresh connection and ' +
    ...
    if (options.ownerId !== undefined && !isPgQueryableKey(options.ownerId)) {
      throw new Error('@qubee/storage: PgDocumentStore ownerId must be lossless JSON text');
    }
  ...
  private requireOwner(operation: string): string {
    if (this.ownerId === null) {
      throw new Error(`@qubee/storage: ${operation} requires an owner-bound document store`);
    }
    return this.ownerId;
  }
  ...
  private async loadStage(
    ...
    if (!isPlainObject(decoded)) {
      throw new Error(
        `@qubee/storage: corrupt stored row for document ${JSON.stringify(stageId)}: ` +
  ...
  private currentVersionError(
    ...
    return new DocumentVersionError(
      ...
      `@qubee/storage: cannot ${operation} document ${JSON.stringify(stageId)} at DSL ` +
  ...
  private validateForSave(
    ...
    for (const scene of doc.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@qubee/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ` +
  ...
  private async persistStage(queryable: Queryable, stageRow: StageRow<TStage>): Promise<void> {
    ...
    if (result.rows.length === 0) {
      throw new DocumentNotFoundError(
        stageRow.id,
        `@qubee/storage: document ${JSON.stringify(stageRow.id)} belongs to another scope`,
      );
    }
  }

  async saveDocument(doc: QubeeDocument<TScene, TStage>): Promise<void> {
    if (isFutureVersioned(doc)) {
      throw new DocumentVersionError(
        doc.stage.id,
        'future',
        doc.dslVersion,
        `@qubee/storage: refusing to save document ${JSON.stringify(doc.stage.id)} — it was ` +
    ...
    await this.transaction(async (queryable) => {
      ...
      if (existingStage && isFutureVersioned(existingStage)) {
        throw new DocumentVersionError(
          ...
          `@qubee/storage: refusing to overwrite document ${JSON.stringify(stageId)} — the ` +
  ...
  async loadDocument(stageId: string): Promise<QubeeDocument<TScene, TStage> | null> {
  ...
  async createFolder(
    ...
    if (!isPgQueryableKey(folderId) || !isPgQueryableKey(name)) {
      throw new Error('@qubee/storage: folder id and name must be lossless JSON text');
    }
  ...
  async putStage(stageId: string, stage: TStage): Promise<void> {
    ...
    if (stage.id !== stageId) {
      throw new Error(
        `@qubee/storage: stage ${JSON.stringify(stage.id)} does not belong to document ` +
          JSON.stringify(stageId),
      );
    }
    ...
    await this.transaction(async (queryable) => {
      ...
      if (!stored) {
        throw new DocumentNotFoundError(
          stageId,
          `@qubee/storage: cannot putStage into missing document ${JSON.stringify(stageId)}`,
        );
      }
  ...
  async putScene(stageId: string, scene: TScene): Promise<void> {
    ...
    await this.transaction(async (queryable) => {
      ...
      if (!stored) {
        throw new DocumentNotFoundError(
          stageId,
          `@qubee/storage: cannot putScene into missing document ${JSON.stringify(stageId)}`,
        );
      }
      if (dslVersionOf(stored) !== DSL_VERSION) {
