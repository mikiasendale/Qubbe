 ...
 */
import type { Stage, Scene } from '@qubee/dsl';
 ...
 */
export interface QubeeDocument<TScene extends SceneLike = Scene, TStage extends Stage = Stage> {
  stage: TStage;
  scenes: TScene[];
  outline?: unknown;
  dslVersion?: string;
...
export class DocumentFolderLimitError extends Error {
  ...
  constructor(readonly limit: number) {
    super(`@qubee/storage: document folder limit reached (${limit})`);
  }
}
...
export interface DocumentStore<TScene extends SceneLike = Scene, TStage extends Stage = Stage> {
   ...
   * {@link putScene}.)
   */
  saveDocument(doc: QubeeDocument<TScene, TStage>): Promise<void>;
   ...
   */
  loadDocument(stageId: string): Promise<QubeeDocument<TScene, TStage> | null>;
