...
import type { QubeeDocument, StageFreshnessManifest } from '../src/document/types.js';
import {
...
if (process.env.STORAGE_PG_CONTRACT_REQUIRED === '1' && !contractUrl) {
  throw new Error(
    '@qubee/storage: STORAGE_PG_CONTRACT_REQUIRED=1 requires PG_CONTRACT_URL; ' +
...
function makeDoc(stageId: string, scenes: { id: string; order: number }[]): QubeeDocument {
  return {
...
describe.skipIf(!contractUrl)('per-scene monotonic revisions (freshness)', () => {
  ...
  it('fault injection: without the triggers the revisions do not move (the signal is DB-borne)', async () => {
    ...
    await pool.query('DROP TRIGGER IF EXISTS qubee_scene_revision_trigger ON document_scenes');
    await pool.query('DROP TRIGGER IF EXISTS qubee_stage_revision_trigger ON document_stages');
    try {
      await pool.query(
        'UPDATE document_scenes SET scene_order = $3 WHERE stage_id = $1 AND id = $2',
        [stageId, 's1', 5],
