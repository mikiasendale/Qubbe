...
Qubee reads environment variables when the server starts. Every setting is optional—enable only what you need. See [`.env.example`](https://github.com/mikiasendale/Qubbe/blob/main/.env.example) in the repository for environment variable examples. See [Supported models](./supported-models.mdx) for built-in model IDs.
...
ComfyUI Image does not require an API key and connects to `http://localhost:8188` by default. Enter the ComfyUI Base URL in Settings, and place workflow JSON exported in API format in Qubee's `public/` directory. Use filenames matching `comfyui-*.json` or containing `workflow`; the Settings page discovers these files automatically and offers them as workflows. With Docker, add the workflow before building the image or mount an individual workflow file into `/app/public/` in the container. Because `comfyui-image` is not a server-managed provider, production deployments connecting to host-side ComfyUI must also set `ALLOW_LOCAL_NETWORKS=true`.
...
The exact course-material formats depend on the selected parser. Qubee currently supports text, PDF, Office documents, images, and some audio/video formats; supported formats and capabilities vary by provider.
...
`unpdf` is built into Qubee and supports basic PDF parsing. For Office documents, images, and material requiring OCR, table, formula, or layout analysis, select a compatible MinerU or AliDocMind provider.
...
Use `MODEL_ROUTES` to choose models for individual generation stages. Unconfigured stages continue to resolve through the client model and `DEFAULT_MODEL`. Its value is a JSON object whose keys are generation stages and whose values can be model strings or objects containing `model` and `thinking`. See [`.env.example`](https://github.com/mikiasendale/Qubbe/blob/main/.env.example) for the complete stage list and examples.
...
```bash
# QUBEE Editor (Pro mode)
NEXT_PUBLIC_QUBEE_EDITOR_ENABLED=true

# Pi chat runtime
NEXT_PUBLIC_PI_CHAT_ENABLED=true

# Vocational education task engine (server-side flag)
QUBEE_ENABLE_VOCATIONAL=true
