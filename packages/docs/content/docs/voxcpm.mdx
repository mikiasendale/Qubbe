...
[VoxCPM2](https://github.com/OpenBMB/VoxCPM) is an open-source TTS model from OpenBMB with voice cloning support. Qubee includes an adapter, so you only need to run VoxCPM on your own machine and connect it.
...
Qubee supports three deployment modes. All three use the same Qubee adapter; select the corresponding backend type in Settings.
...
Open **Settings → Text to speech → VoxCPM2**, choose the backend type, and paste the Base URL. The Request URL preview below shows the actual address Qubee will call.
...
During synthesis, Qubee dynamically generates a voice prompt from each Agent's persona. No configuration is required; this is what you get by doing nothing.
