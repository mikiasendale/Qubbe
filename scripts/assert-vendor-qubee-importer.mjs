 ...
 *
 * The app loads the parser at runtime from `/vendor/qubee-importer/index.js` — a
 * gitignored build artifact that `scripts/sync-qubee-importer.mjs` copies into
...
const entry = path.join(root, 'public/vendor/qubee-importer/index.js');
...
try {
  ...
  console.error('[assert-vendor] It is loaded at runtime via /vendor/qubee-importer/index.js');
  console.error('[assert-vendor] and is produced by the postinstall sync step. To fix:');
  console.error(
    '[assert-vendor]   pnpm --filter @qubee/importer build && pnpm run sync:qubee-importer',
  );
