import {
  ...
  readManifest,
} from './qubee-packages.mjs';

/**
 * An owned @qubee package may be declared as a dependency of another exactly
 ...
 * two copies of `@qubee/dsl`. The dsl carries the schema, the validators and
...
const OWNED = new Set(QUBEE_PACKAGES.map((name) => `@qubee/${name}`));
...
if (listProblems.length > 0) {
  report('The shared @qubee package list has drifted:', listProblems);
}
...
for (const name of QUBEE_PACKAGES) {
  ...
  for (const [dependency, range] of Object.entries(manifest.dependencies ?? {})) {
    ...
    if (range === 'workspace:^') {
      console.log(`@qubee/${name}: dependencies.${dependency} = ${range}`);
      continue;
    }
    failures.push(
      `@qubee/${name} declares dependencies."${dependency}" as ${JSON.stringify(range)}. ` +
  ...
  for (const field of FORBIDDEN_FIELDS) {
    for (const dependency of Object.keys(manifest[field] ?? {})) {
      if (!OWNED.has(dependency)) continue;
      failures.push(
        `@qubee/${name} declares ${field}."${dependency}". An owned @qubee package may ` +
...
for (const [name, expected] of Object.entries(INTERNAL_DEPENDENTS)) {
  ...
  if (observed === undefined) {
    failures.push(
      `@qubee/${name} no longer declares any owned @qubee dependency in \`dependencies\`; ` +
        `it was expected to depend on ${expectedDependencies.join(', ')}. If that is intended, update ` +
        'INTERNAL_DEPENDENTS in scripts/qubee-packages.mjs — this check must not go quiet ' +
        'on its own.',
    );
    continue;
  }
  ...
  if (
    ...
    failures.push(
      `@qubee/${name} declares owned dependencies (${observedDependencies.join(', ')}), but ` +
...
for (const [name, observed] of seen) {
  ...
  failures.push(
    `@qubee/${name} declares the owned dependency ${observed.join(', ')} but is absent from ` +
      'INTERNAL_DEPENDENTS in scripts/qubee-packages.mjs, so nothing checks how it is ' +
...
for (const name of QUBEE_PACKAGES) {
  for (const dependency of Object.keys(readManifest(name).devDependencies ?? {})) {
    if (!OWNED.has(dependency)) continue;
    failures.push(
      `@qubee/${name} declares devDependencies."${dependency}". An owned @qubee package ` +
...
if (failures.length > 0) {
  report('Internal @qubee dependency declarations are not in the required shape:', failures);
}
