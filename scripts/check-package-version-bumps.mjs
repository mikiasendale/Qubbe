...
import { QUBEE_PACKAGES, assertPackageListIsComplete } from './qubee-packages.mjs';
...
const ignoredPackageInputs = Object.fromEntries(
  QUBEE_PACKAGES.map((name) => [name, ignoredInputOverrides[name] ?? commonIgnoredInputs]),
);
 ...
 * used them.
 */
const DSL_VERSION_SOURCES = ['packages/@qubee/dsl/src/version.ts'];
 ...
 *
 * The dependents declare `@qubee/dsl` as `workspace:^`, published as a
...
function runDiffMode(base) {
  ...
  failIfAny(
    failures,
    'Every @qubee publishable package change must ship with a new package version:',
  );
...
function runReleaseMode() {
  ...
  failIfAny(failures, 'Refusing to publish @qubee packages:');
