...
/**
 * The one list of owned, publishable @qubee packages.
 *
 ...
 */
export const QUBEE_PACKAGES = ['dsl', 'generation', 'storage', 'renderer', 'editor', 'importer'];

/** The packages that depend on other owned packages, and on which ones. */
export const INTERNAL_DEPENDENTS = {
  generation: ['@qubee/dsl'],
  storage: ['@qubee/dsl'],
  renderer: ['@qubee/dsl'],
  editor: ['@qubee/dsl', '@qubee/renderer'],
  importer: ['@qubee/dsl'],
};

export const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');

export const PACKAGES_DIRECTORY = join(repositoryRoot, 'packages/@qubee');

export function packageDirectory(name) {
  return `packages/@qubee/${name}`;
}
...
export function assertPackageListIsComplete() {
  ...
  const onDisk = readdirSync(PACKAGES_DIRECTORY, { withFileTypes: true })
    ...
    .sort();
  const listed = [...QUBEE_PACKAGES].sort();

  for (const name of onDisk) {
    if (!listed.includes(name)) {
      problems.push(
        `packages/@qubee/${name} exists but is not in QUBEE_PACKAGES, so every check ` +
  ...
  for (const name of listed) {
    if (!onDisk.includes(name)) {
      problems.push(`QUBEE_PACKAGES lists ${name}, but packages/@qubee/${name} is gone.`);
    }
  }
...
function describeSetDifference(expected, observed, subject) {
  ...
  for (const name of observed) {
    if (!expected.includes(name)) {
      problems.push(`${subject} names ${name}, which is not in QUBEE_PACKAGES.`);
    }
  }
  return problems;
}
 ...
 * (the build, pack, publish and tag loops). `--filter "@qubee/<name>"` is
...
function crossCheckPublishWorkflow(workflow) {
  ...
  // up `packages/@qubee/$pkg/package.json` inside the shell loops, where
  ...
  if (!pathsBlock) {
    ...
    const triggerPaths = [
      ...pathsBlock[1].matchAll(/packages\/@qubee\/([^/"'\s]+)\/package\.json/g),
    ].map((match) => match[1]);
    problems.push(
  ...
  const filtered = new Set(
    [...source.matchAll(/--filter\s+"?@qubee\/([^"\s]+)"?/g)].map((match) => match[1]),
  );
  for (const name of QUBEE_PACKAGES) {
    if (!filtered.has(name)) {
      problems.push(
        `${PUBLISH_WORKFLOW} never passes --filter "@qubee/${name}", so it is never built ` +
          'or tested on the release path.',
      );
    }
  }
  for (const name of filtered) {
    if (!QUBEE_PACKAGES.includes(name)) {
      problems.push(
        `${PUBLISH_WORKFLOW} filters @qubee/${name}, which is not in QUBEE_PACKAGES.`,
      );
    }
  }
