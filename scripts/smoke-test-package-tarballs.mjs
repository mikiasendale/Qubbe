...
import { INTERNAL_DEPENDENTS, QUBEE_PACKAGES, readManifest } from './qubee-packages.mjs';
...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'qubee-package-smoke-'));
...
/**
 * The dependents declare `@qubee/dsl` as `workspace:^`, which pnpm publishes
 * as `^<dsl version>`. That range is what lets a consumer installing several
 * @qubee packages together resolve ONE copy of the dsl. An exact pin — which
...
function assertDeduplicableDslRange(name, manifest, dslVersion, ownedPackages) {
  const range = manifest.dependencies?.['@qubee/dsl'];
  assert(
    range !== undefined,
    `@qubee/${name} no longer declares @qubee/dsl; update this check if that is intended`,
  );
  assert.equal(
    range,
    `^${dslVersion}`,
    `@qubee/${name} publishes @qubee/dsl as ${JSON.stringify(range)} rather than ` +
  ...
  for (const field of ['peerDependencies', 'optionalDependencies']) {
    ...
    assert.deepEqual(
      declared,
      [],
      `@qubee/${name} publishes ${field} entries for owned packages ` +
  ...
  console.log(`@qubee/${name} publishes @qubee/dsl as ${range}, and nowhere else.`);
}

try {
  const packageNames = QUBEE_PACKAGES;
  const localPackages = new Set(packageNames.map((name) => `@qubee/${name}`));
  const tarballs = Object.fromEntries(
    packageNames.map((name) => {
      const { version } = readManifest(name);
      return [name, join(artifactDirectory, `qubee-${name}-${version}.tgz`)];
    }),
  );
  ...
  for (const name of packageNames) {
    ...
    for (const [peer, range] of Object.entries(manifest.peerDependencies ?? {})) {
      ...
      assert(
        ...
        `@qubee packages declare different ${peer} peer ranges: ${existing} and ${range}`,
      );
    ...
    JSON.stringify(
      {
        ...
        dependencies: {
          ...peerDependencies,
          ...Object.fromEntries(
            packageNames.map((name) => [`@qubee/${name}`, `file:${tarballs[name]}`]),
          ),
        },
      },
      null,
...
import { fileURLToPath } from 'node:url';
import { RUNTIME_DSL_VERSION, validateRuntimeSession } from '@qubee/dsl';
import { PROMPT_IDS, buildPrompt } from '@qubee/generation';
import { DOCUMENT_PG_SCHEMA } from '@qubee/storage';
import { SlideCanvas } from '@qubee/renderer';
import { createEditorTransaction as createEditorTransactionFromRoot } from '@qubee/editor';
import { createEditorTransaction } from '@qubee/editor/core';
import { EMPTY_SELECTION } from '@qubee/editor/react';
import { EditableSlideCanvasWithUI } from '@qubee/editor/ui';
...
const importerEntry = fileURLToPath(import.meta.resolve('@qubee/importer'));
assert((await stat(importerEntry)).isFile());
const importerRequireEntry = createRequire(import.meta.url).resolve('@qubee/importer');
...
for (const subpath of [
  ...
  await import(\`@qubee/storage/\${subpath}\`);
}
`,
  );
  run('node', ['smoke.mjs'], { cwd: consumerDirectory });

  console.log('Validated @qubee package tarball imports passed.');
} finally {
