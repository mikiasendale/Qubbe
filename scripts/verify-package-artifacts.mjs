...
import { QUBEE_PACKAGES, readManifest } from './qubee-packages.mjs';

const DIGESTS_FILE = 'SHA256SUMS';
const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');

function expectedArtifacts() {
  return QUBEE_PACKAGES.map((shortName) => {
    const manifest = readManifest(shortName);
    return {
      shortName,
      name: `@qubee/${shortName}`,
      version: manifest.version,
      filename: `qubee-${shortName}-${manifest.version}.tgz`,
    };
  });
}
