---
name: qubee
description: Qubee assistant for setting up, generating, and extending Qubee. Use when the user wants to use Qubee, generate a multi-agent interactive classroom, or build on / extend / customize Qubee and its @qubee/* SDK (secondary development, 二开) — covers local setup, startup modes, provider keys, classroom generation, and secondary development (forking, providers/storage/themes, routes, or the renderer/editor).
user-invocable: true
...
- Do not assume the OpenClaw agent's own model or API key will be reused by Qubee.
- Qubee classroom generation uses Qubee server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Qubee server-side config files may control provider selection and defaults.
...
Ask the user how they want to use Qubee:

1. **Run locally** — Clone the repo, configure provider keys, and run on your machine.
2. **Extend or build on Qubee (二次开发)** — Fork the repo and customize the product, or consume the `@qubee/*` SDK to build something new.

If the user chooses local mode, proceed to phase 1 as usual.
If the user chooses to extend/build on Qubee, load [references/extend.md](references/extend.md) and skip the setup/generation phases.

### 1. Clone Or Reuse Existing Repo

Load [references/clone.md](references/clone.md).

Use this when the user has not installed Qubee yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Qubee

After the user has chosen a startup mode and configured keys, start Qubee using the chosen method, then verify the service with `GET {url}/api/health`.
