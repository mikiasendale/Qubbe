...
- Qubee is healthy at the selected `url`
...
- Do not try to recover from auth, provider, model, or base URL errors by changing request parameters. Tell the user to fix Qubee server-side config and retry only after they confirm.
