...
Qubee generation does not automatically reuse the OpenClaw agent's current model or API key.

Qubee server APIs resolve their own model and provider keys from Qubee server-side config.

This skill does not rely on runtime overrides for model, provider, API key, base URL, or provider type.

If the user wants to change any of those, they must edit Qubee server-side config files.
...
- Qubee has **no hardcoded model fallback**. If `DEFAULT_MODEL` is unset (and no client model is sent), generation **fails with an error** rather than silently picking a default. So the user must always set `DEFAULT_MODEL` explicitly to match whichever provider key they configured.
...
Do not recommend bare model IDs such as `gemini-2.5-flash` by themselves, because Qubee will otherwise parse them as OpenAI models.
...
- "I recommend configuring Qubee through `.env.local` first. Please edit that file locally and tell me when you're done."
