...
export interface FakeDocumentStore {
  ...
  docs: Map<string, QubeeDocument<AppScene, AppStage>>;
  ...
  saveCalls: QubeeDocument<AppScene, AppStage>[];
...
export function createFakeDocumentStore(): FakeDocumentStore {
  const docs = new Map<string, QubeeDocument<AppScene, AppStage>>();
  const stageRevs = new Map<string, number>();
  const sceneRevs = new Map<string, Map<string, number>>();
  const saveCalls: QubeeDocument<AppScene, AppStage>[] = [];
  ...
  const store = {
    forOwner: () => store,
    async saveDocument(doc: QubeeDocument<AppScene, AppStage>) {
    ...
    async putStage(stageId: string, stage: AppStage) {
      const doc = docs.get(stageId);
      if (!doc) throw new Error('@qubee/storage: document not found');
