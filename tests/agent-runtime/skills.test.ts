...
describe('pi-native skills', () => {
  ...
  it('activates a skill only after its SKILL.md is read successfully', async () => {
    const root = mkdtempSync(join(tmpdir(), 'qubee-native-skill-'));
    try {
...
describe('shipped skill constraints', () => {
  ...
  it('ships the K-12 core-literacy skill as an Qubee-native classroom flow', async () => {
...
describe('toPosixPath', () => {
  ...
  it('converts Windows backslashes to forward slashes', () => {
    const windowsPath = 'C:\\repo\\Qubee\\skills\\agent-runtime\\build-personal-skill';
    const posix = windowsPath.split('\\').join('/');
    expect(posix).toBe('C:/repo/Qubee/skills/agent-runtime/build-personal-skill');
  });

  it('handles mixed separators', () => {
    const mixed = 'C:\\repo/Qubee\\skills/SKILL.md';
    const posix = mixed.split('\\').join('/');
    expect(posix).toBe('C:/repo/Qubee/skills/SKILL.md');
  });
});
...
describe('normalizeSkillFileInfo', () => {
  it('normalizes both path and name from a Windows-shaped FileInfo', () => {
    const info = {
      name: 'C:\\repo\\Qubee\\skills\\agent-runtime\\quiz\\SKILL.md',
      path: 'C:\\repo\\Qubee\\skills\\agent-runtime\\quiz\\SKILL.md',
      isFile: true,
      isDirectory: false,
      size: 1024,
    };
    const result = normalizeSkillFileInfo(info);
    expect(result.path).toBe('C:/repo/Qubee/skills/agent-runtime/quiz/SKILL.md');
