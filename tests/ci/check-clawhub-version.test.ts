...
const publishScriptPath = resolve(repositoryRoot, '.github/scripts/publish-qubee-skill.sh');
const workflowPath = resolve(repositoryRoot, '.github/workflows/publish-qubee-skill.yml');
...
describe('check-clawhub-version', () => {
  ...
  it('routes preview and publish through the same shared script', () => {
    ...
    expect(
      workflow.match(/bash \.github\/scripts\/publish-qubee-skill\.sh --dry-run/g),
    ).toHaveLength(1);
    expect(
      workflow.match(/^\s+bash \.github\/scripts\/publish-qubee-skill\.sh$/gm),
    ).toHaveLength(1);
    expect(workflow.match(/bash -n \.github\/scripts\/publish-qubee-skill\.sh/g)).toHaveLength(
      2,
    );
    expect(workflow.match(/- "\.github\/scripts\/publish-qubee-skill\.sh"/g)).toHaveLength(2);
  ...
  it('runs automatic and manual paths in a no-secret macOS Bash 3.2 job', () => {
    ...
    expect(
      compatibilityJob.match(/\/bin\/bash \.github\/scripts\/publish-qubee-skill\.sh/g),
    ).toHaveLength(2);
  ...
  it('gates publish on main and preserves checkout and stale-tree safeguards', () => {
    ...
    const publishSequence = [
      ...
      String.raw`exit 1`,
      String.raw`fi`,
      String.raw`if ! git cat-file -e "HEAD\^\{tree\}:skills/qubee" 2>/dev/null; then`,
      String.raw`handle_divergence "skills/qubee was deleted"`,
      String.raw`fi`,
      String.raw`if ! git cat-file -e "origin/main\^\{tree\}:skills/qubee" 2>/dev/null; then`,
      String.raw`handle_divergence "skills/qubee was removed from main"`,
      String.raw`fi`,
      String.raw`source_tree="\$\(git rev-parse HEAD:skills/qubee\)"`,
      String.raw`main_tree="\$\(git rev-parse origin/main:skills/qubee\)"`,
      String.raw`if \[\[ "\$source_tree" != "\$main_tree" \]\]; then`,
      String.raw`handle_divergence "skills/qubee changed on main"`,
      String.raw`fi`,
      String.raw`bash \.github/scripts/publish-qubee-skill\.sh`,
    ].join(String.raw`\s+`);
    expect(publishJob).toMatch(new RegExp(publishSequence));
    expect(publishJob.match(/publish-qubee-skill\.sh/g)).toHaveLength(2);
