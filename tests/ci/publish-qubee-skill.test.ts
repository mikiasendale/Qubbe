...
const publishScript = resolve(repositoryRoot, '.github/scripts/publish-qubee-skill.sh');
...
function runPublish(options: RunOptions = {}) {
  ...
  Object.assign(env, {
    ...
    SOURCE_REPO: options.sourceRepo ?? 'mikiasendale/Qubbe',
...
describe('publish-qubee-skill shell contract', () => {
  ...
  it('rejects unsupported arguments', () => {
    ...
    expectFailure(usage.result, 'Usage: publish-qubee-skill.sh [--dry-run]');
  ...
  it.each([
    ...
    expectFailure(result, 'Usage: publish-qubee-skill.sh [--dry-run]');
