...
const editorReadmePath = resolve(process.cwd(), 'packages/@qubee/editor/README.md');
...
describe('renderer editor app boundary', () => {
  ...
  it('documents a complete third-party controlled-state and style setup', () => {
    ...
    expect(readme).toContain("import '@qubee/renderer/fonts.css';");
  ...
  it('smoke-tests every published editor entry from packed artifacts', () => {
    ...
    expect(smoke).toContain("from '@qubee/editor';");
    expect(smoke).toContain("from '@qubee/editor/core';");
    expect(smoke).toContain("from '@qubee/editor/react';");
    expect(smoke).toContain("from '@qubee/editor/ui';");
  });
});
