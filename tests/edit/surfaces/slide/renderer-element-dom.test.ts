...
import {
  ...
  QUBEE_ELEMENT_ID_ATTRIBUTE,
  SCREEN_ELEMENT_ID_PREFIX,
  editableElementDomId,
  qubeeElementIdAttributes,
  screenElementDomId,
...
describe('renderer editor DOM contract', () => {
  ...
  it('stamps the renderer-agnostic pick attribute accepted by hit-testing', () => {
    expect(QUBEE_ELEMENT_ID_ATTRIBUTE).toBe('data-qubee-element-id');
    expect(qubeeElementIdAttributes('title-1')).toEqual({ 'data-qubee-element-id': 'title-1' });
    expect(INTERACTION_ELEMENT_ID_ATTRIBUTES[0]).toBe(QUBEE_ELEMENT_ID_ATTRIBUTE);
