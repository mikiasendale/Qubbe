...
vi.mock('@qubee/generation', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@qubee/generation')>()),
...
describe('scene API retry boundary', () => {
  ...
  it('preserves the provider status carried by a PBLGenerationError', async () => {
    ...
    const { PBLGenerationError } = await import('@qubee/generation');
