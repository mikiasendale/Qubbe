...
import type { RuntimeRecord } from '@qubee/dsl';
import { BrowserRuntimeStore } from '@qubee/storage';
...
const resolvedModel = { provider: 'test.provider', modelId: 'qubee-resolved-model' };
const envNames = [
  'NEXT_PUBLIC_PI_CHAT_ENABLED',
  'QUBEE_ENABLE_PI_NATIVE_CHILD_RUNTIME',
  'QUBEE_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT',
  'TAVILY_API_KEY',
  'TAVILY_BASE_URL',
...
describe('PR2 Native Child route production wiring', () => {
  beforeEach(() => {
    ...
    process.env.QUBEE_ENABLE_PI_NATIVE_CHILD_RUNTIME = 'true';
    process.env.QUBEE_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT = 'true';
  ...
  it('keeps the production route on Legacy when the Native runtime flag is absent', async () => {
    delete process.env.QUBEE_ENABLE_PI_NATIVE_CHILD_RUNTIME;
    delete process.env.QUBEE_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT;
    const directorResponses = [
      [toolCall('read-1', 'read_scene', { sceneId: 'scene-current' }), finish('tool-calls')],
      [
        toolCall('delegate-1', 'call_agent', {
