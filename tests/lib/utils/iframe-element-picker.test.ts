...
afterEach(() => {
  ...
  delete (window as unknown as Record<string, unknown>).__qubeeElementPickerInstalled;
});
...
describe('iframe element picker shim', () => {
  it('stays dormant, blocks armed page clicks, emits picks, syncs pins, and exits', () => {
    ...
    expect(document.querySelector('[data-qubee-element-picker-overlay]')).toBeNull();
    ...
    flushFrame();
    expect(document.querySelector('[data-qubee-element-picker-overlay]')).not.toBeNull();
    ...
    flushFrame();
    expect(document.querySelector('[data-qubee-picker-pin]')?.textContent).toBe('1');

    window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }));
    expect(document.querySelector('[data-qubee-element-picker-overlay]')).toBeNull();
    expect(postMessage).toHaveBeenCalledWith(
      { __qubeeInteractive: true, kind: 'element-picker-disarmed' },
      '*',
    );
  });
});
