...
import { HttpRuntimeStoreError } from '@qubee/storage/runtime/http';
...
describe('chat RuntimeStore cutover', () => {
  ...
  it.each([
    [
      ...
      `@qubee/storage: cannot append to session "chat:stage-chat:anon%3Achat-test:session-1" with status 'completed' — records may only be appended to an active session`,
    ],
    [
      'post-race reclassification',
      `@qubee/storage: session "chat:stage-chat:anon%3Achat-test:session-1" is no longer active; its current status is 'completed'`,
    ],
