...
import { DSL_VERSION } from '@qubee/dsl';
import { BrowserRuntimeStore, type KVStore, type RuntimeStore } from '@qubee/storage';
...
describe('database runtime chat integration', () => {
  ...
  it('loads a divergent destination without marking or deleting its legacy source', async () => {
    ...
    const { getDocumentStore } = await import('@/lib/document-store');
    const { loadStageData } = await import('@/lib/utils/stage-storage');
    const kv = new (await import('@qubee/storage')).BrowserKVStore();
  ...
  it('upgrades past the abandoned lease schema and adds compound chat restore staging', async () => {
    ...
    const intermediate = new Dexie('QUBEE-Database', {
      indexedDB: globalThis.indexedDB,
  ...
  it('acquires both the stage-wide and legacy partition Web Lock names', async () => {
    ...
    expect(requested).toEqual([
      'qubee:chat-storage:all',
      `qubee:chat-storage:${encodeURIComponent('stage-compatible-lock')}`,
      `qubee:chat-storage:${encodeURIComponent(`stage-compatible-lock\0${learnerKey}`)}`,
    ]);
  });
  ...
  it('clears document and runtime databases without Web Locks', async () => {
    ...
    localStorage.setItem('qubee:device:document-migration:stage-clear-document', '{}');
    localStorage.setItem('qubee:device:editor-current-scene:stage-clear-document', '{}');
    await getAssetPool().put(new Blob(['private generated media'], { type: 'text/plain' }));
    expect((await indexedDB.databases()).map((entry) => entry.name)).toContain('qubee-asset-pool');
    ...
    expect(localStorage.getItem('qubee:device:document-migration:stage-clear-document')).toBeNull();
    expect(
      localStorage.getItem('qubee:device:editor-current-scene:stage-clear-document'),
    ).toBeNull();
