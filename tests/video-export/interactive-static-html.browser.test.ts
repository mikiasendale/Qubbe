    ...
    try {
      ...
      await page.waitForFunction(() => {
        ...
        const timelines = (window as typeof window & { __timelines?: Record<string, unknown> })
          .__timelines;
        return (
          host?.getAttribute('data-interactive-static-state') === 'frozen' && !!timelines?.qubee
        );
      });
      ...
      const before = await child.evaluate(() => ({
        ...
        state: document.documentElement.getAttribute('data-qubee-static-state'),
      ...
      const result = await page.evaluate(() => {
        ...
        const diagnostics = (
          window as typeof window & {
            __qubeeVideoDiagnostics?: Array<{ code: string; message: string }>;
          }
        ).__qubeeVideoDiagnostics;
        const manifest = (
          window as typeof window & {
            __qubeeVideoManifest?: { runtimeDiagnostics?: Array<{ code: string }> };
          }
        ).__qubeeVideoManifest;
        const runtimeReport = document.querySelector<HTMLElement>(
          '[data-qubee-runtime-diagnostics]',
        );
        return {
