...
describe('searchWithBaidu', () => {
  ...
  it('queries only enabled Baidu sub-sources and merges their results', async () => {
    proxyFetchMock.mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          ...
          result: {
            lemma_title: 'Qubee',
            lemma_url: 'https://baike.baidu.com/item/Qubee',
    ...
    expect(proxyFetchMock.mock.calls.map((call) => String(call[0]))).toEqual([
      'https://appbuilder.baidu.com/v2/baike/lemma/get_content?search_type=lemmaTitle&search_key=Qubee',
      'https://qianfan.baidubce.com/v2/tools/baidu_scholar/search?wd=Qubee&pageNum=0&enable_ai_abstract=true',
    ]);
  ...
  it('defaults Baidu sub-sources to all enabled when omitted', async () => {
    ...
    await searchWithBaidu({ query: 'Qubee', apiKey: 'baidu-key' });
